...
export async function GET(request: NextRequest) {
  connectDB()
  let response = await Transportsetting.find({})
...
export async function DELETE(request: NextRequest) {
  ...
  let response = await Transportsetting.findOneAndDelete({ _id: _id })
    .then((sub) => { return [{ data: sub, class: "success", msg: "Transportsetting deleted successfully" }, { status: 200 }] })
    .catch((error) => { return [{ error: error,data:{}, class: "error" }, { status: 500 }] })
  return NextResponse.json(response[0], response[1]);
}