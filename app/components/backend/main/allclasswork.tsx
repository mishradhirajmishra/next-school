...
import { CreateUpdateClassWorkApi, DelClassWorkApi, GetAllClassWorkApi, GetTeacherPeriodAllotedApi } from '../../shared/utils/apis';
...
interface AllClassWorkProps {
  newData: CHWorkType,
  reciveDataTobeEdited: (option: CHWorkType) => void
}

const AllClassWork: FC<AllClassWorkProps> = ({ newData, reciveDataTobeEdited }) => {
  ...
  useEffect(() => { getClassWork() }, [newData])
  const getClassWork = async () => {
    let data = await GetAllClassWorkApi()
  ...
  const deleteClassWork = async (val: Delobj) => {
    if (val.delete) {
      let data = await DelClassWorkApi(val._id)
  ...
  const updateStatus = async (val: any) => {
    let data = await CreateUpdateClassWorkApi(val)
    ...
    <>
      ...
      <div className="mt-5 relative   overflow-x-auto pb-5">
        <table >
          ...
          </> : <tbody><tr><td colSpan={11} className='text-center c-text-dark'>No Class Work found.</td></tr></tbody>}
        </table>
