...
import { GetPeriodoptionApi,  UpdatePeriodOptionApi} from '../../shared/utils/apis';
const initialValue = SelectOptioninit

const AllPeriodOption: FC = () => {
  ...
  useEffect(() => { if (rowsBacup.length = 1) { getPeriodoption() } }, [])
  ...
  const updateTimeoption = async (val: SelectOptionType) => {
    val.hide = !val.hide
    let data = await UpdatePeriodOptionApi(val)
    getPeriodoption()
    ...
    <>
      ...
      <div className="mt-5 relative   overflow-x-auto pb-5">
        <table >
            ...
            {rows && (rowsPerPage > 0
              ...
              <tbody key={pr._id}>
                <tr >
                  ...
                  <td>{pr.name}</td>
                  <td className='max-w-[100px] whitespace-nowrap'>
          ...
          </> : <tbody><tr><td colSpan={11} className='text-center c-text-dark'>No Timeoption found.</td></tr></tbody>}
...
export default AllPeriodOption;