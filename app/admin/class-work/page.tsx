...
import { CreateUpdateClassWorkApi, GetTeacherPeriodAllotedApi } from "@/app/components/shared/utils/apis"
...
import AllClassWork from "@/app/components/backend/main/allclasswork"
...
const Page = () => {
  ...
  const onSubmit = async (values: any) => {   
    let data = await CreateUpdateClassWorkApi(values)   
    ...
    <>
      ...
      <div className="flex flex-col w-full overflow-y-auto  h-[calc(100vh-48px)]  px-10 py-5 ">
        <div className="flex gap-10 ">
          <h1 className="block text-2xl font-bold  c-text-light"><span className="c-text-success">Class</span>-Work</h1>
          <button type="button" className="btn-outline-light rounded-full" onClick={() => { setOpenDrawer(true) }}> <PlusIcon />  Add </button>
        </div>
      <AllClassWork newData={newData} reciveDataTobeEdited={reciveDataTobeEdited} /> 
      </div>
