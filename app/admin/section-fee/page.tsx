...
import { CreateUpdateFeeApi, CreateUpdateSectionFeePatchApi, } from "@/app/components/shared/utils/apis"
import { FeeType, Feeinit, FeevalidationSchema, classDataProps } from "@/app/components/shared/model/feehelper"
...
import AllSectionFee from "@/app/components/backend/main/allsectionfee"
...
const Page = () => {
  ...
  const onSubmit = async (values: any) => {
    if (classData) {
      const body = { class_id: classData.class_id, section_id: classData.section_id, fee: values }
      const data = await CreateUpdateSectionFeePatchApi(body)
  ...
  const reciveDataTobeEdited = (data: FeeType, classData: classDataProps) => {
    ...
    <>
      ...
      <div className="flex flex-col w-full overflow-y-auto  h-[calc(100vh-48px)]  px-10 py-5 ">
        <div className="flex gap-10 ">
          <h1 className="block text-2xl font-bold  c-text-light"><span className="c-text-success">Section </span>Fee</h1>
          {/* <button className="btn-outline-light rounded-full" onClick={() => { setOpenDrawer(true); setformValues(Feeinit) }}> <PlusIcon />  Add </button> */}
        </div>
        <AllSectionFee newData={newData} reciveDataTobeEdited={reciveDataTobeEdited} />
      </div>
      <aside className={`${openDrawer ? "w-full" : "w-0"} duration-0 cursor-pointer z-0 right-0 absolute min-h-[calc(100vh-48px)]`}>
        ...
        <div className={`${openDrawer ? drawerWidth : "w-0"} c-bg-dark cursor-default z-10 duration-500 pt-5 right-0 absolute h-[calc(100vh-48px)]`}>
          <div className={`${openDrawer ? drawerWidth : "hidden"}`}>
            <TabBar tabs={["Section Fee change"]} onOptionClick={(val) => { setActiveTab(val) }} />
              ...
              {({ values, errors, touched, setFieldValue, handleChange }) => <>
                <Form>
                  <Tabcontainer show={activeTab == 0} >
                    {classData?.class !=undefined && classData?.section  !=undefined &&  <Breadcrumb options={[classData?.class, classData?.section]}/>}
